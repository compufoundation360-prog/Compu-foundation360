...
import { CheckCircle2, AlertCircle, Zap, Cpu, MemoryStick, CircuitBoard, HardDrive, Info, List, Monitor } from 'lucide-react';
            ...
            <div className="p-2 border-b border-border bg-background/50">
                <div className="grid grid-cols-2 p-1 bg-muted rounded-lg">
                    <button
                        ...
                        className={cn(
                            "flex items-center justify-center gap-2 text-xs font-bold py-1.5 rounded-md transition-all",
                            activeTab === 'specs' ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"
                        )}
                    >
                        <Monitor className="w-3 h-3" /> specs
                    </button>
                    <button
                        onClick={() => setActiveTab('parts')}
                        className={cn(
                            "flex items-center justify-center gap-2 text-xs font-bold py-1.5 rounded-md transition-all",
                            activeTab === 'parts' ? "bg-background text-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"
                        )}
                    >
                        <List className="w-3 h-3" /> parts
                    </button>
                </div>
            </div>
                            ...
                            {Object.entries(placedParts).length === 0 ? (
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="flex justify-between items-center bg-muted/50 p-2 rounded">
                                        ...
                                        <span className="font-mono font-bold text-sm">${totalCost}</span>
                                    </div>
                                </div>
                            )}
                        </div>
